...
function BTP_POP(x::Vector{PolyVar{true}},f::Polynomial{true},g::Vector{Polynomial{true,Float64}},h::Vector{Polynomial{true,Float64}},k::Int64,R::Float64;ak::Float64=0.0,EigAlg="Arpack",tol=1e-5,showNormGrad=false)
  ...
  @time begin
   ...
   if ak==0 
    ...
    @simd for j in 1:m
        @inbounds a[:,j]=makevec(A[j].*ak,l_vec,sqrt2,IndVec)
    end    
   ...
   c=makevec_dense(C.*ak,l_vec,sqrt2,IndVec)     
    ...
    function phi(nvar::Cint,xp::Ptr{Cdouble},gp::Ptr{Cdouble})
        ...
        return(convert(Cdouble,eigval+zvar[nvar]))
    end
    ...
    G=makemat_dense(c-a*z,l_vec,s,sqrt2,reIndVec)./ak
